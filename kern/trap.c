...
trap_init(void)
{
    ...
    void th_irq_ide();
    void th_irq_error();
    // Initialize trap handlers without error code
    SETGATE(idt[T_DIVIDE], 0, GD_KT, t_divide, 0);
    SETGATE(idt[T_NMI], 0, GD_KT, t_nmi, 0);
    SETGATE(idt[T_BRKPT], 0, GD_KT, t_brkpt, 3);  //allow user to use breakpoint
    SETGATE(idt[T_OFLOW], 0, GD_KT, t_oflow, 0);
    SETGATE(idt[T_BOUND], 0, GD_KT, t_bound, 0);
    SETGATE(idt[T_ILLOP], 0, GD_KT, t_illop, 0);
    SETGATE(idt[T_DEVICE], 0, GD_KT, t_device, 0);
    // Initialize trap handlers with error code
    ...
    // System call - allow user access (DPL=3)
    SETGATE(idt[T_SYSCALL], 0, GD_KT, t_syscall, 3);

	//Extra Credit
	// SETGATE(idt[T_DEBUG], 0, GD_KT, handler_debug, 0);

	// initialize the IDT entries for IRQs 0-15
...
trap_dispatch(struct Trapframe *tf)
{
    ...
    switch (tf->tf_trapno) {
        ...
        case T_DEBUG:
            //clear trap flag and enter monitor
            tf->tf_eflags &= ~FL_TF;
            monitor(tf);
            return;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	//check if the environment has registered a page fault handler
    if (curenv->env_pgfault_upcall) {

        struct UTrapframe *utf;
        
        //determine where to put the UTrapframe
        if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) {

            //already on the exception stack, make room for a new UTrapframe
            //leave an empty 32-bit word at the top
            utf = (struct UTrapframe *)(tf->tf_esp - sizeof(struct UTrapframe) - 4);
        } 
		
		else {

            //not on exception stack, start at the top of the exception stack
            utf = (struct UTrapframe *)(UXSTACKTOP - sizeof(struct UTrapframe));
        }
        
        //check if we're still in the valid part of the exception stack
        user_mem_assert(curenv, (void*)utf, sizeof(struct UTrapframe), PTE_W);
        
        //fill in the UTrapframe
        ...
        //set up the environment to run the user-level page fault handler
        //make sure to set up the stack correctly with the UTrapframe
